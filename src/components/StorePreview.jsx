...
const StorePreview = ({ product }) => {
  ...
  const { setShoping_cart, shoping_cart, showAlert } = useDashboard();
  ...
  const handleAdd = () => {
    ...
    const exist = shoping_cart.filter((el) => el.id === item.id);
    ...
    if (exist.length) {
      ...
      setShoping_cart([...shoping_cart, item]);
    }
  ...
  return (
    <div className="p-2 w-56 m-4">
      ...
      <p className={`text-sm font-bold text-center text-sky-500 uppercase`}>
        $ {price}
      </p>
      <button
        disabled={!avalible}
        className={`p-1 ${
          avalible ? 'bg-sky-500' : 'bg-gray-400'
